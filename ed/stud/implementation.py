...
from stud.event_detect import ModelParams
from stud.event_detect import EventDetectionModel
...
class StudentModel(Model):
    ...
    def __init__(self, 
        ...
        self.token_vocab = torch.load(token_vocab_path)
        self.label_vocab = torch.load(label_vocab_path)

        params = ModelParams(self.token_vocab, self.label_vocab)

        self.ed_model = EventDetectionModel(params).to(device)

        self.ed_model.load_state_dict(torch.load(model_path))
    ...
    def encode_inputs(self, tokens: List[List[str]]) -> List[List[int]]:
        ...
        for sample in tokens:
            e_tokens = []
            for token in sample:
                id_ = self.token_vocab[token] if token in self.token_vocab else self.token_vocab[unk_token]
                e_tokens.append(id_)
    ...
    def decode_outputs(self, predictions: List[List[int]]) -> List[List[str]]:
        ...
        for sample in predictions:
            labels = []
            for id_ in sample:
                label = list(self.label_vocab.keys())[list(self.label_vocab.values()).index(id_)]
                print("label: ", label)
                labels.append(label)
